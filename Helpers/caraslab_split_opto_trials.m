...
for i = 1:numel(files)
    ...
    for j = 1:numel(Session)
        ...
        if opto_index > 0
            ...
            fprintf('\tOpto tag name not found. Adding an Opto tag column just so analyses don''t crash, but make sure to double-check this...\n')
            optoTag = optoTagNames;
            temp_tableSession.(optoTagNames) = zeros(size(temp_tableSession, 1), 1);
        end
